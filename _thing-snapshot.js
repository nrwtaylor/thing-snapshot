  ...
  */
  ...
  fs.readFile("/var/www/kplex-thing/snapshot.json", "utf8", (err, data) => {
    if (err) {
      ...
      /*
const pingInput= "";;
  fs.readFile("/tmp/snapshot-ping.json", "utf8", (err, data) => {
    if (err) {
      pingInput = `Error reading file from disk: ${err}`;
    } else {
      pingInput = data;

try {
pingParsed = JSON.parse(pingInput);
} catch (e) {
pingParsed = {error:'JSON parse error'};
}
*/
